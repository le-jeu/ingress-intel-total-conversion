...
// IITC.addHook('portalAdded', function(data) { log.log(data) });
//
...
var isRunning = 0;
IITC.runHooks = function(event, data) {
  if (!hooks[event]) { return true; }
  var interrupted = false;
  isRunning++;
  $.each(hooks[event], function (ind, callback) {
    try {
...
IITC.addHook = function(event, callback) {
...
IITC.removeHook = function(event, callback) {
  if (typeof callback !== 'function') {
    throw new Error('Callback must be a function.');
  }

  var listeners = hooks[event];
  if (listeners) {
